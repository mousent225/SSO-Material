...
namespace SSO_Material.Repositories
{
    public class ForgetCardRepository
    {
        ...
        public string UpdateForgetCard(ForgetCardModel model)
        {
            try
            {
                _db.SP_FORGETCARD_UPDATE(model.EmpId, model.WorkDate, model.TimeIn, model.TimeOut, model.SecurityName, model.TemporaryCard, model.GateNumber, model.Reason, model.ReasonOthers);
                return "Ok";
            }
            catch (Exception ex)
            {
